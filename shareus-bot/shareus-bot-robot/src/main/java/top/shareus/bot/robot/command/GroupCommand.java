...
public final class GroupCommand extends JRawCommand {
	...
	public void onCommand(@NotNull CommandContext context, @NotNull MessageChain args) {
		...
		try {
			...
			log.info(args.get(0) + "\t请求到了：" + members.size() + "名成员的信息");
			List<NormalMemberVO> collect = members.stream().map(NormalMemberUtils::toMemberVO).collect(Collectors.toList());
