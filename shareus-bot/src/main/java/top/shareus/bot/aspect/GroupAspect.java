...
public class GroupAspect {
    ...
    @Around(value = "@annotation(group)", argNames = "joinPoint,group")
    public Object around(ProceedingJoinPoint joinPoint, Group group) throws Throwable {
        for (Object arg : joinPoint.getArgs()) {
            if (arg instanceof MessageEvent event) {
                long id = event.getSubject().getId();
                if (group.groupId() == id || hasIt(group.groupList(), id)) {
                    // 恢复程序执行
